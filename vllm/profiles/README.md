...
| `meta-models--Muse-Glimmer-30B` | ✅ | BF16 ~56 GiB. Spezial-Image `vllm/vllm-openai:muse-glimmer` (registriert `muse_glimmer`, arm64). Parser `muse_glimmer` tool+reasoning. **max_tokens ≥ 1024** — das Modell denkt ~380 Tok vor jeder Antwort, sonst `content=null` bei `finish=length` |
| `RedHatAI--Muse-Glimmer-30B-NVFP4` | ✅ | NVFP4 ~22 GiB, **empfohlene Variante auf GB10** (Rezept). Gemessen 2026-08-13: Start 331 s, TTFT 0.33–0.43 s, 12.3 tok/s — gegen BF16 mit 4.2 tok/s bei gleicher Konfiguration (CUDA-Graphs beidseitig). Das Modell ist speicherbandbreitenlimitiert: 273 GB/s ÷ Gewichtsgroesse ergibt 12.4 bzw. 4.9 tok/s theoretisch, gemessen 12.3 bzw. 4.2. Kleiner = schneller, eager/Graphs aendern nichts. Gleiches Spezial-Image |
| `inclusionAI--Ling-3.0-tiny-fp8` | ❌ | Laedt auf Nightly (BailingMoeV3 ist erst nach v0.27.1 upstream), rechnet aber **still falsch**: „2+2=3", Flagge „blau/schwarz/gold". 5 Backend-/Quant-Konfigurationen getestet, alle falsch |
| `inclusionAI--Ling-3.0-flash-fp4` | ❌ | Gemischt quantisiert (Basis fp8 + routed experts mxfp4). Upstream-vLLM laedt das nicht: Shape-Mismatch in `routed_experts._load_w2` (6 vs 24). FP4 ist laut Modellkarte SGLang-only |
| `nvidia--Llama-4-Scout-17B-16E-Instruct-NVFP4` | ✅ | NVFP4 (modelopt), 109B gesamt / 17B aktiv, ~61 GiB. Gemessen 2026-08-13: Start 191 s, TTFT 0.15–0.23 s, **19.2 tok/s** — schnellstes bisher gemessenes Modell hier. Marlin-Env wie Nemotron-3-Super (FLASHINFER_CUTLASS hat auf sm_120 keine Tactic). `max_model_len` 32768 statt der 10M aus der config.json; das Auto-Profil schaetzte 8 GB statt 61 GiB und haette OOM verursacht |
| `nvidia--Llama-3.3-70B-Instruct-NVFP4` | ⏳ | NVFP4 (modelopt), dicht, ~40 GiB. Profil geschrieben, Erststart steht aus. Auto-Profil schaetzte 140 GB und begrenzte den Kontext auf 8192 — beides falsch. Erwartung: ~6.8 tok/s (273 GB/s ÷ 40 GiB) |
